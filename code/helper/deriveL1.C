...
void deriveL1(string algo, string algo2, string era) {
  ...
  double vpar2[npar2] = {1.45292e-01, -2.26787e-01, 3.93751e-01, -8.30810e-02};
  ...
  {-5.191,-4.889,-4.716,-4.538,-4.363,-4.191,-4.013,-3.839,-3.664,-3.489,
    ...
    TFile *f = new TFile(Form("Parameter_%s.root",a));
    assert(f && !f->IsZombie());
    ...
    for (int ieta = 0; ieta != neta; ++ieta) {
      ...
      if(g2 == NULL)   // if not enough statistics, sometimes graph will be not present
         continue;
      assert(g2);
    ...
    if (batch) c4->SaveAs(Form("deriveL1_filtered_%s.pdf",a));

    for_each(algo.begin(), algo.end(), 
            [](char &ch){
              ch = ::toupper(static_cast<unsigned char>(ch));
            });

    const char *aCAP = algo.c_str();
    const char *_algo = algo2.c_str();
    const char *_era = era.c_str();

    // Print out text file
    ofstream fout(Form("%s_L1FastJet_%s.txt",_era,_algo));
